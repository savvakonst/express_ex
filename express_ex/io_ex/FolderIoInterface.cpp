...
FolderIoInterface::~FolderIoInterface()
{
    FolderIoInterface::close();
}
...
bool FolderIoInterface::removeDataset(const char* name){
    ...
    for(auto &i : datasets_){
        if(i!=nullptr)
            if(getNameByPtr(i) == std::string(name))
                return false;
    }
...
std::string FolderIoInterface::getNameByPtr(const QFile* obj_id){
    return obj_id->fileName().toStdString();
}
