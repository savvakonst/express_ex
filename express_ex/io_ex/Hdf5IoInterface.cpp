...
Hdf5IoInterface::~Hdf5IoInterface(){
    Hdf5IoInterface::close();
}
...
DatasetsStorage_ifs::id_t Hdf5IoInterface::openDataset(const char* name){
    ...
    id_t ret = 0;
    /*
    //TODO: it is wrong way since... what if I want to have two separated interfaces to reading file
    ...
    */
    ...
    hid_t   id = H5Dopen1(file_id_, name);
...
bool Hdf5IoInterface::removeDataset(const char* name){
    ...
    for( auto& i : datasets_){
        if (i.id_>-1)
            if(getNameByHid(i.id_) == std::string(name))
                return false;
    }
...
{
    if(ds_ssize_t(datasets_.size()) < index)
        return -1;
...
bool Hdf5IoInterface::initVariables()
{
    ...
    if(::H5Pset_chunk(prop_list_id_, 1, &chunks) < 0)
        return false;
